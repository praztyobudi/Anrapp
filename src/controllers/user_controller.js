...
export const createUser = async (req, res) => {
  try {
    const { name, password, username, department, role } = req.body;
    if (!name || !password || !username || !department || !role) {
      return errorResponse(res, 'Please provide all required fields', 400)
    }
    const user = await userService.createUser(name, password, username, department, role);
    //rename user_role to role
    const { user_role, ...rest } = user;
    const renameRole = { ...rest, role: user_role }; //semua field selain user_role

    return successResponse(res, 'created successfully', renameRole)
